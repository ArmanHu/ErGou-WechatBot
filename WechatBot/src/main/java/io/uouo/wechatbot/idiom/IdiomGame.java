...
public class IdiomGame implements Runnable {
    ...
    public synchronized void run() {
    	...
    	try {
	        ...
	        GAME_LIST.add(new GameNode(idiomNo, "System", firstIdiom));
	        ...
	        while(idiomNo<=maxIdiomNo) {
	            ...
	            try {
		    		...
		    		for (GameNode gameNode : GAME_LIST) {
		    			if(message.equals(gameNode.getIdiom())){
            	...
            	resetCounter();
	            GAME_LIST.add(new GameNode(idiomNo, wxid, message));
    ...
    public void buildGameResult(){
        ...
        for(GameNode gameNode : GAME_LIST){
        	String a = gameNode.getWxid();
        	//剔除第一次系统出题的次数
        	if("System".equals(a)){
        		continue;
        	}
        	String nickName = MyFileUtil.getNickNameByWxid(a);
            //统计重复数的个数
